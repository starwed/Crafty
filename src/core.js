    ...
    trigger: function (event, data) {
        ...
        if (this.length === 1) {
            ...
            if (h && h[this[0]]) {
                ...
                metadata.depth++;
                for (i = 0; i < l; i++) {
                    if (typeof callbacks[i] === "undefined" && metadata.depth<=1) {
...
Crafty.extend({
    ...
    trigger: function (event, data) {
        ...
        // (To learn how the handlers object works, see inline comment at Crafty.bind)
        var hdl = eventHandler(event), metadata = hdl.data,
