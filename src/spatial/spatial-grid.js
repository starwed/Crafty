...
HashMap.prototype = {
    ...
    search: function (rect, filter) {
        ...
        for (i = keys.x1; i <= keys.x2; i++) {
            ...
            for (j = keys.y1; j <= keys.y2; j++) {
                if ((cell = this.map[(i << 16) ^ j])) {
                    for (let obj of cell) results.add(obj);
                }
            }
        }
        ...
        if (filter) {
            ...
            for (let obj of results) {
                var objBounds = obj._cbr || obj._mbr || obj;
                // filter out results that don't intersect
                if (!(objBounds._x < rect._x + rect._w && objBounds._x + objBounds._w > rect._x &&
                    objBounds._y < rect. _y + rect._h && objBounds._y + objBounds._h > rect._y))
                    results.delete(obj);
            }
        }
    ...
    refresh: function (entry) {
        ...
        for (i = keys.x1; i <= keys.x2; i++) {
            for (j = keys.y1; j <= keys.y2; j++) {
                cell = this.map[(i << 16) ^ j];
                if (!cell) cell = this.map[(i << 16) ^ j] = new Set();
                cell.add(obj);
            }
        }
    ...
    traverseRay: function (origin, direction, callback) {
        ...
        while ((minCol <= currentCol && currentCol <= maxCol) &&
            ...
            if ((cell = this.map[(currentCol << 16) ^ currentRow])) {
                ...
                for (let cellObj of cell) {
                    // if supplied callback returns true, abort traversal
                    if (callback(cellObj, previousDistance))
                        return;
                }
            }
