...
if [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "my-travis" ]; then
  echo -e "Uploading nightly build to Crafty-Distro\n" 
  ...
  git clone -b nightlies https://git@github.com/craftyjs-machine-user/Crafty-Distro.git $HOME/nightlies
  ...
  git commit -m 'automated commit of build $TRAVIS_BUILD_NUMBER'
