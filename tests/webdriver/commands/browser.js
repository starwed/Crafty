var q = require("q"), jimp = require("jimp");
      ...
      function(filePath, bounds) {
        return this.saveScreenshot().then(function(screenshotBuffer, response) {
          return jimp.read(screenshotBuffer).then(function(screenshot) {
            ...
            var x = bounds.x > 0
              ? Math.min(bounds.x, rotatedCrop.stretchW - 1)
              : 0,
              y = bounds.y > 0
                ? Math.min(bounds.y, rotatedCrop.stretchH - 1)
                : 0,
              w = bounds.w > 0
                ? Math.min(bounds.w, rotatedCrop.stretchW - x)
                : rotatedCrop.stretchW - x,
              h = bounds.h > 0
                ? Math.min(bounds.h, rotatedCrop.stretchH - y)
